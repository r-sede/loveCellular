...
hero.getBootomColl = function(this)
  return this.x * BLOCKSIZE * PPM + 0.3 * PPM * BLOCKSIZE, this.y * BLOCKSIZE * PPM + 1 * PPM * BLOCKSIZE, 0.4 *
    BLOCKSIZE *
    PPM, 0.05 * BLOCKSIZE * PPM
end

hero.getUpColl = function(this)
  return this.x * BLOCKSIZE * PPM + 0.3 * PPM * BLOCKSIZE, this.y * BLOCKSIZE * PPM + 0.5 * PPM * BLOCKSIZE, 0.4 *
    BLOCKSIZE *
    PPM, 0.05 * BLOCKSIZE * PPM
end

hero.getLeftColl = function(this)
  return this.x * BLOCKSIZE * PPM + 0.2 * PPM * BLOCKSIZE, this.y * BLOCKSIZE * PPM + 0.7 * PPM * BLOCKSIZE, 0.1 *
    BLOCKSIZE *
    PPM, 0.2 * BLOCKSIZE * PPM
end

hero.getRightColl = function(this)
  return this.x * BLOCKSIZE * PPM + 0.7 * PPM * BLOCKSIZE, this.y * BLOCKSIZE * PPM + 0.7 * PPM * BLOCKSIZE, 0.1 *
    BLOCKSIZE *
    PPM, 0.2 * BLOCKSIZE * PPM
end
...
hero.update = function(this, dt)
  ...
  if this.idle == false then
    ...
    if this.animTimer <= 0 then
      ...
      this.stepSound = (this.stepSound + 1) % 2
      if not footSound:isPlaying() and this.stepSound == 0 then
        footSound:play()
      end
    end
  end
  ...
  for yy = startY, endY do
    for xx = startX, endX do
      if
        terrain[yy][xx] == 1 or terrain[yy][xx] == 257 or terrain[yy][xx] == 256 or terrain[yy][xx] == 28 or
          terrain[yy][xx] == 199 or
          terrain[yy][xx] == 255 or
          terrain[yy][xx] == 135 or
          terrain[yy][xx] == 143 or
          terrain[yy][xx] == 207 or
          terrain[yy][xx] == 31 or
          terrain[yy][xx] == 30 or
          terrain[yy][xx] == 15 or --
          terrain[yy][xx] == 47 or --
          ...
          terrain[yy][xx] == 62 or
          terrain[yy][xx] == 231 or
          terrain[yy][xx] == 124 or
          terrain[yy][xx] == 7 or
          terrain[yy][xx] == 60 or
          terrain[yy][xx] == 252 or
          terrain[yy][xx] == 253 or
          terrain[yy][xx] == 247 or
          terrain[yy][xx] == 167 or
          --
          terrain[yy][xx] == 156 or
          --
          terrain[yy][xx] == 183 or
          --
          terrain[yy][xx] == 156
       then
        local cx, cy, cwidth, cheight = this:getLeftColl()
        if
          rafUtils.isCollideRec(
            cx,
            cy,
            cwidth,
            cheight,
...
hero.draw = function(this)
  love.graphics.draw(
    this.atlas,
    this.sprite[this.direction][this.frame],
